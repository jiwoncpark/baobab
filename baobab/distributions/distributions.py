...
def sample_normal(mu, sigma, lower=-np.inf, upper=np.inf, log=False):
...
def eval_normal_pdf(eval_at, mu, sigma, lower=-np.inf, upper=np.inf, log=False):
...
def sample_multivar_normal(mu, cov_mat, is_log=None, lower=None, upper=None):
