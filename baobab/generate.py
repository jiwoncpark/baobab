...
def parse_args():
	...
	parser.add_argument('--n_data', default=None, dest='n_data', type=int,
		help='size of dataset to generate (overrides config)')
	parser.add_argument('--dest_dir', default=None, dest='dest_dir', type=str,
		help='destination for output folder (overrides config)')
...
def main():
	...
	if (cfg.bnn_omega.kinematics.calculate_vel_disp or
		cfg.bnn_omega.time_delays.calculate_time_delays):
		for_cosmography = True
	else:
		for_cosmography = False
	imager = Imager(cfg.components, lens_mass_model, src_light_model,
		...
		for_cosmography=for_cosmography,
		magnification_frac_err=cfg.bnn_omega.magnification.frac_error_sigma)
	# Initialize BNN prior
	if for_cosmography:
		kwargs_lens_eqn_solver = {'min_distance': 0.05,
		'search_window': cfg.instrument['pixel_scale']*cfg.image['num_pix'],
		'num_iter_max': 100}
		bnn_prior = getattr(bnn_priors, cfg.bnn_prior_class)(cfg.bnn_omega,
			cfg.components, kwargs_lens_eqn_solver)
	else:
	...
	while current_idx < cfg.n_data:
		...
		# select on sampled model parameters
		if selection.reject_initial(sample):
			continue
		# Generate the image
		img, img_features = imager.generate_image(sample, cfg.image.num_pix,
			cfg.survey_object_dict)
		...
		if 'agn_light' in cfg.components:
			...
			meta['measured_magnification'] = img_features[
				'measured_magnification'].tolist()
		...
		if current_idx == 0:
			...
			# init empty df for next checkpoint chunk
			metadata = pd.DataFrame()
			gc.collect()
